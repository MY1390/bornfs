...
case class Case(
    var row: ArrayBuffer[(Attr, Value)],
    val classLabel: Value,
    val frq: Int
) {
...
case class Dataset(
    raw_data: Seq[(ArrayBuffer[(Attr, Value)], Value)],
    sort: Int,
    tutorial: Boolean,
    verbose: Boolean
) {
  ...
  val data =
    temp_dict.par.map { x => Case(x._1._1, x._1._2, x._2) }.to[ArrayBuffer]
  ...
  if (tutorial) {
    println
    println(
      "The current dataset includes " + nSamples + " samples described by " +
        (maxAttr + 1) + " features identified by 0 through 5."
    )
    println("Each sample is labeled by an integer in [0, " + maxLabel + "].")
    println(
      "Samples with the same feature values and labels are agregated into a single case object, "
    )
  ...
  def entropyPrefixPlus(index: Index): (Double, Double) = {
    ...
    val tempVal = partitions.par.map { p =>
      val temp =
        Array.fill(maxVal(entity(index)) + 1)(Array.fill(maxLabel + 1)(0))
      for (i <- p) {
        val c = data(i)
        temp(c(index))(c.classLabel) += c.frq
      }
      (
        temp.map(x => -xlog2(x.sum)).sum,
        temp.map(x => x.map(-xlog2(_)).sum).sum
      )
    }
    (
  ...
  val entropyEntire = count.values
    .filter(_ > 0)
    .par
    .map { x => -x * log2(x) }
    .sum / nSamples + log2(nSamples)
  ...
  val entropyEntireLabel = count.values
    .filter(_ > 0)
    .par
    .map { x => -x * log2(x) }
    .sum / nSamples + log2(nSamples)
  ...
  if (tutorial) {
    ...
    println(
      "- The entropy of the class variable H(Class) is " + f.format(
        entropyLabel
      ) + "."
    )
    ...
    println(
      "H(F)= " + (0 to maxAttr)
        .map { i => i + ":" + f.format(entropyAttr(i)) }
        .mkString(" ")
    )
    println
    println(
      "- The scores of mutual information of individual features to labels are:\n"
    )
    println(
      "I(F;C)= " + (0 to maxAttr)
        .map { i =>
          i + ":" + f.format(
            entropyAttr(i) + entropyLabel - entropyAttrLabel(i)
          )
        }
        .mkString(" ")
    )
    println
    println(
      "- The entropy of the entire features H(Entire) is " + f.format(
        entropyEntire
      ) + "."
    )
    println(
      "- The mutual information of the entire features to labels I(Entire;Class) is " +
  ...
  def sortVal(index: Index): Double = {
    val (hfp, hfpc) = entropyPrefixPlus(
      index
    ) // (H(F, Prefix), H(F, Prefix, C))
    // val very_large = 1000.0
  ...
  def sortAttrs: (Array[Int], Array[Int]) = {
    ...
    val tmp =
      (0 to lim).par.map(i => (i, sortVal(i))).toArray.sortWith(_._2 > _._2)
    ...
    if (tutorial) {
      print(
        "The features F in the search range are sorted in a decreasing order of "
      )
      sort match {
        case 0 =>
          print("the estimated ratio of relevance gain to noise gain: ")
          println(
            "[I(F;Selected,Class)-I(F;Selected)] / [H(F)-I(F;Selected,Class]."
          )
        case 1 =>
          println(
            "the estimated negative noise gain: I(F;Selected,Class)-H(F)."
          )
        case 2 =>
          println(
            "the estimated relevance gain: I(F;Selected,Class)-I(F;Selected)."
          )
        case _ =>
      }
      ...
      println(
        (0 to lim)
          .map { i => entity(i) + "(" + f.format(tmp(i)._2) + ")" }
          .mkString("", " ", "\n")
      )
  ...
  def select(delta: Double, hop: Int): Seq[Attr] = {
    ...
    if (tutorial) {
      ...
      println(
        "BornFS is a greedy but fast algorithm to find an optimal feature set FS that yields:"
      )
      ...
      println(
        "it iterates selection of a single feature until it reaches a final answer."
      )
      println(
        "Each iteration is associated with a search range, an ordered set of features."
      )
      println(
        "The high efficiency of BornFS when selecting a feature from the search range is due to leverage of binary search."
      )
      println("The search range shrinks as feature seelction proceeds.")
      println(
        "BornFS stops when the search range becomes empty and returns the features selected at the moment as a final answer."
      )
      println(
        "Features in search ranges are sorted with the frequency specified by an option (-h)."
      )
      println(
        "The frequency affects the quality of ouputs by and the time efficiency of BornFS."
      )
      println(
        "To be more specific, we can expect that the quality will improves and the time efficiency decreases with higher frequency."
      )
      print("In this run of BornFS, ")
      if (hop == maxAttr + 1) {
        println(
          "sorting features is executed only once at the first iteration."
        )
      } else {
      ...
      println(
        "In the following, we describe the operations performed in each iteration."
      )
    }
    ...
    while (true) {
      ...
      if (tutorial) {
        ...
        println(
          "Each case object of the dataset belongs to exactly one partition."
        )
        println(
          "Case objects in the same partition have the same values for the features selected at the moment."
        )
        if (prefix.isEmpty) {
          println(
            "Since no features have been selected when the first iteration starts, the iteration has only a single partition, which consists of all the case objects."
          )
        }
        println(
          "In each partition, the case objects belonging to it are sorted in a lexicographical order of values of the features of the sorted search range: "
        )
        println
        for (p <- partitions) {
          println(">> Partition")
          p.foreach { i =>
            ...
            print(
              (prefix ++ (0 to lim))
                .map(a => entity(a) + ">" + c.value(entity(a)))
                .mkString(" ")
            )
            println(" label>" + c.classLabel + " frq>" + c.frq)
          }
        ...
        println
        println(
          "Selection of a feature is conducted with respect to importance indices of the features in the search range."
        )
        ...
        println(
          "* Mutual information of the entire features to labels = I(Entire;Class)."
        )
        ...
        println(
          (0 to lim)
            .map { i =>
              entity(i) + ":" + f.format(ratio(i - 1))
            }
            .mkString("", " ", "\n")
        )
        println(
          "BornFS selects the right-most feature subject to the condition that the ratio is smaller than the given threshold, in this case, " + delta + "."
        )
      }
      ...
      if (i < 0) {
        ...
        if (tutorial) {
          ...
          println
          println(
            "The relevance and noise of the currently selected features are:"
          )
          val relevance = entropyPrefix + entropyLabel - entropyPrefixLabel
          val noise = entropyPrefix - relevance
          println(
            "* Relevance I(Selected;Class) = " + f.format(relevance) + ";"
          )
          println("* Noise H(Selected|Class) = " + f.format(noise) + ".")
          println(
            "The harmonic and geometric means of I(S;C)/I(E;C) and I(S;C)/H(FS) are computed as:"
          )
          println(
            "* mu_H = " + f.format(
              2 * relevance / (miEntireLabel + entropyPrefix)
            ) + ";"
          )
          println(
            "* mu_G = " + f.format(
              relevance / math.sqrt(miEntireLabel * entropyPrefix)
            ) + "."
          )
        } else if (verbose) {
          print("." + entity(i) + ".")
