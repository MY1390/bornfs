...
case class ARFFReader(filename: String) {
  ...
  def removeUnselectedAttrs(selected_attrs: List[Symbol]) {
    val remove_list =
      ((for (attr <- selected_attrs)
        yield (attr2index(attr))) ::: List(instances.numAttributes - 1)).toArray
...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[MainOption]("Born Feature Selection") {
      opt[String]('i', "input") required () valueName ("<path>") action {
        (x, o) =>
          o.copy(in = x)
      ...
      opt[String](
        's',
        "sort"
      ) valueName ("<ratio, noise, relevance, difference, harmonic>") action {
        (x, o) =>
          o.copy(sort = x)
      ...
      opt[Boolean]('T', "tutorial") valueName ("<true,false>") action {
        (x, o) =>
          o.copy(tutorial = x)
    ...
    var out: String = ""
    var log: String = "low"
    var sort: Int = 0 // ratio of relevance to noise
    ...
    println(
      "The minimum ratio of mutual information of selected features to the maximum is " + threshold + "."
    )
    if (hop == 0) {
    ...
    if (tutorial) {
      println(
        "The processing of feature selection will be explained step by step (tutorial mode on)."
      )
    } else {
    ...
    if (verbose) {
      println(
        "States of execution will be displayed real time (vabose mode on)."
      )
    } else {
    ...
    val db: ARFFReader = ARFFReader(in)
    val data: ArrayBuffer[Tuple2[ArrayBuffer[Tuple2[Int, Int]], Int]] =
      db.sparse_instances.to[ArrayBuffer].map { x =>
        (x._1.map { y => (db.attr2index(y._1), y._2) }, x._2)
      }
    println("finished.")
    println(
      "Found " + db.numInstances + " instances and " + db.numAttrs + " features including class."
    )
    ...
    if (tutorial && (db.numInstances > 10 || db.numAttrs > 20)) {
      println
      println(
        "Tutorial mode is canceled because the dataset is too big (#Instance > 10 or #Feature > 20)."
      )
      tutorial = false
    }
    ...
    val ds: Dataset = Dataset(data, sort, tutorial, verbose)

    val timeCreateObject: Long = System.nanoTime() - start
    ...
    if (log == "high" || log == "low") {
      ...
      val log_file_name = in + (sort match {
        ...
        case 3 => "-diff-"
        case 4 => "-muh-"
      }) + threshold + "-" + (if (hop >= db.numAttrs - 1) 0
                              else hop) + "-bornfs.log"
      val log_file =
        new OutputStreamWriter(new FileOutputStream(log_file_name), "utf-8")
//      val log_file = new OutputStreamWriter(new FileOutputStream(inWOext+".log"), "utf-8")
      ...
      log_file.write(
        "## Hop = " + (if (hop >= db.numAttrs - 1) 0 else hop) + "\n"
      )
      log_file.write("## Level of detail of log = " + log + "\n")

      log_file.write("\n# Run-time\n")
      log_file.write(
        "## Creating a dataset object = " + fns.format(timeCreateObject) + "\n"
      )
      log_file.write(
        "## Sorting features = " + fns.format(ds.timeSortFeatures) + "\n"
      )
      log_file.write(
        "## Sorting instances = " + fns.format(ds.timeSortInstances) + "\n"
      )
      log_file.write(
        "## Selecting features = " + fns.format(ds.timeSelectFeatures) + "\n"
      )
      ...
      if (log == "high") {
        ...
        log_file.write(
          (0 to ds.maxAttr)
            .map { i =>
              db.index2attr(i) + ":" + f.format(ds.entropyAttr(i))
            }
            .mkString(" ")
        )
        log_file.write("\n\n")
        ...
        log_file.write(
          (0 to ds.maxAttr)
            .map { i =>
              db.index2attr(i) + ":" + f.format(ds.entropyAttrLabel(i))
            }
            .mkString(" ")
        )
        log_file.write("\n\n")
